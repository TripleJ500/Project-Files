...
$ID2 = isset($_POST['coursePrefix']) ? $_POST['coursePrefix'] : NULL;
$response = isset($_POST['response']) ? $_POST['response'] : NULL;

if ($ID == '' && $ID2 == '' && $response == 0) 
{ 
    ?> 
        <form action = "GradeInfoQueryForm.html" method = "post">
...
{
    $i = 0;
    $sql = "SELECT grade.gradeID, student.lastName, student.firstName, student.major, grade.studentID, 
            coursePrefix, semester, grade FROM student RIGHT JOIN grade 
            ON student.studentID = grade.studentID";
    $query = $db -> query($sql);
    $headers = array("Grade ID", "Last Name", "First Name", "Major", "Student ID",
...
elseif($ID != NULL)
{
    ...
    $sql = "SELECT grade.gradeID, student.lastName, student.firstName, student.major, grade.studentID, 
            coursePrefix, semester, grade FROM student RIGHT JOIN grade
            ON student.studentID = grade.studentID WHERE student.studentID = ?";   
